use fflonk_verifier::{verify, VerificationKey, Proof, Public};
use hex_literal::hex;

#[test]
fn test_readme_example() {
    // The proof data from the README example
    let data = hex!(
        r#"
        
        "#
    );
    
    // Create the verification key (default for the example)
    let vk = VerificationKey::default();
    
    // Parse the proof from the hex data
    let proof = Proof::try_from(&data).unwrap();
    
    // Create the public input
    let pubs: Public = hex!("0d69b94acdfaca5bacc248a60b35b925a2374644ce0c1205db68228c8921d9d9").into();

    // Verify the proof
    let result = verify(&vk, &proof, &pubs);
    
    // Assert that verification succeeds
    assert!(result.is_ok(), "Proof verification failed: {:?}", result);
} 