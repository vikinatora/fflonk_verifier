...
async function verifyProof(proofBytes, publicInput) {
  try {
    ...
    const data = contract.methods.verify(proofBytes, publicInput).encodeABI();
    ...
    const txObject = {
      ...
      gasPrice: web3.utils.toHex(Math.floor(Number(gasPrice) * 1.1)) // 10% higher than current gas price
    };
    ...
    // Check the result
    const result = await contract.methods.last_verification_result().call();
    ...
    // Try to revert reason if available
    if (error.reason) {
